SRC_MAIN_PROTO_DIR=grpc/src/main/proto/th2_grpc_lw_data_provider
GITHUB_TH2=github.com/th2-net

TH2_LW_DATA_PROVIDER=th2-lw-data-provider
TH2_LW_DATA_PROVIDER_URL=$(GITHUB_TH2)/$(TH2_LW_DATA_PROVIDER)@2.13.0-dev
...
generate-grpc-files:
	$(eval $@_PROTO_DIR := $(shell go list -m -f '{{.Dir}}' $(TH2_LW_DATA_PROVIDER_URL))/$(SRC_MAIN_PROTO_DIR))
	protoc \
		--go_out=$(MODULE_DIR) \
